...
# @url_param tagging_params=&utm_source=compari&utm_medium=cpc&utm_campaign=direct_link (TAGGING_PARAMS) 
...
if(!file_exists($site_base_path . "../config.php")) {
	exit('<HTML><HEAD><TITLE>404 Not Found</TITLE></HEAD><BODY><H1>Not Found</H1>Please ensure that datafeed_compari_opencart.php is in the same folder with config.php file or define the path to config.php file in $site_base_path variable.</BODY></HTML>');
}
else {
...
$tagging_params = (@$_GET['tagging_params'] != "") ? urldecode($_GET['tagging_params']) : "utm_source=compari&utm_medium=cpc&utm_campaign=direct_link";
...
if (@$_GET['get'] == "options") {
	...
	echo "<b>Add custom Tagging to product URL</b> - possible values url_encode(TAGGING_PARAMS) default value tagging_params=utm_source=compari&utm_medium=cpc&utm_campaign=direct_link<br />";
	echo "tagging_params=utm_source=compari&utm_medium=cpc&utm_campaign=direct_link (TAGGING_PARAMS) <a href=\"" . $script_basepath . "?tagging_params=from%3Dprice" . "\" >" . $script_basepath . "?tagging_params=from%3Dprice" . "</a><br /><br />";
...
if ($oc_version == "1.4") {
	...
	,ifnull((SELECT GROUP_CONCAT(text SEPARATOR ', ')  FROM " . DB_PREFIX . "product_attribute left join " . DB_PREFIX . "attribute  on " . DB_PREFIX . "attribute.attribute_id=" . DB_PREFIX . "product_attribute.attribute_id where " . DB_PREFIX . "product_attribute.product_id=" . DB_PREFIX . "product.product_id and " . DB_PREFIX . "attribute.attribute_group_id=15),'') as 'Subcategory'
...
//		print_r($COMB_ARR);
		...
		if ($show_combinations == "on" && is_array($COMB_ARR) && sizeof($COMB_ARR) > 0) {
			...
			//print 
			$file = fopen("../datafeed_compari.csv","w");
			$listAll .= $prod_manufacturer . $datafeed_separator .
				$prod_name . $datafeed_separator .
				$prod_url . $datafeed_separator .
				round($prodDB_price*1.24, 2 ). $datafeed_separator .
				$prod_image . $datafeed_separator .
				$category_name . $datafeed_separator .
                                $subcategory. $datafeed_separator .  
				$prod_desc. $datafeed_separator .
		...
		//print $listAll;
		file_put_contents ("../datafeed_compari.csv",$listAll); 
		//$list = array($listAll);
