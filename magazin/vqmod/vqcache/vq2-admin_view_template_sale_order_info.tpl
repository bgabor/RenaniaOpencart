    ...
    <div class="content">
      ...
      <div id="tab-history" class="vtabs-content">
        ...
        <table class="form">
         ...
         <tr id="voucher" style="display: none;">
            <input type="hidden" id="already_have_voucher" value ="<?php echo $already_have_voucher ; ?>" >
            <td>&nbsp;</td>
            <td><a id="addVoucher"> Adauga voucher </a></td>
         </tr>
...
<script type="text/javascript"><!--
    ...
    $('select[name="order_status_id"]').change(function(){
      ...
      if ( (status_id == 3 || status_id == 5 ) && $('#already_have_voucher').val() == 0)
      {
         $("#voucher").show();
         $('#addVoucher').click(function(){

         $.ajax({
               url: 'index.php?route=sale/voucher/addVoucher&token=<?php echo $this->request->get['token']; ?>&order_id=<?php echo $this->request->get['order_id']; ?>',
               type: 'POST',
               dataType: 'json',
               beforeSend: function(){},
               success: function( response ) {
               
                  removeOpencartMessages();
                  insertOpencartMessages( response['message'], 'success' );
                  
                  $('#already_have_voucher').val( response['already_have_voucher'] );                
                  $("#voucher").hide();
               },
               failure: function(){},
               error: function(){}
      ...
      else if ( $('#already_have_voucher').val() > 0 )
      {
         $("#voucher").show();
         $("#addVoucher").html( 'A fost deja adaugat voucher!' );
      }
      else
      {
         $("#voucher").hide();
       ...
       $.ajax({
            url: 'index.php?route=extension/openbay/populateMailContent&token=<?php echo $this->request->get['token']; ?>&order_id=<?php echo $this->request->get['order_id']; ?>&status_id='+$('select[name="order_status_id"]').val(),
            type: 'post',
            dataType: 'html',
            beforeSend: function(){},
            success: function(html) {
    ...
    function removeOpencartMessages()
    {
        $( "#container #content div.breadcrumb" ).next( "div.warning" ).each(function(){ $( this ).remove(); });
        $( "#container #content div.breadcrumb" ).next( "div.success" ).each(function(){ $( this ).remove(); });       
    }
    
    function insertOpencartMessages( messageForInsert, typeOfMessage )
    {
        $( "#container #content div.breadcrumb" ).after( '<div class="' + typeOfMessage + '">' + messageForInsert + '</div>' );        
