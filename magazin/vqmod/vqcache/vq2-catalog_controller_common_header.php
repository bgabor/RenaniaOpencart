...
$this->data['text_welcome'] = str_replace('b2b.renania.ro','magazin.renania.ro',$this->data['text_welcome']);
		$this->data['text_logged'] = sprintf($this->language->get('text_logged'), $this->url->link('account/account', '', 'SSL'), $this->customer->getFirstName(), $this->url->link('account/logout', '', 'SSL'));
		//$this->data['text_b2b_login'] = $this->url->link('account/login/index', 'type=B2B', 'SSL');
		$this->data['text_b2b_login'] = B2B_HTTP_SERVER."login";
		...
		$this->data['header_B2B_tab'] = FALSE;

		/*if($_SERVER['REMOTE_ADDR'] == "5.2.202.87") {
			//die($_SERVER['SERVER_NAME']);
			//die(strpos($_SERVER['SERVER_NAME'],'renania'));
			die($_SERVER['SERVER_NAME'].' that is 2 it '.strpos($_SERVER['SERVER_NAME'], 'renania.ro'));
		}*/

		if(in_array( $this->customer->getCustomerGroupId(), array( 3,4 )) || (isset($this->request->get['type']) && $this->request->get['type'] == 'B2B') || strpos($_SERVER['SERVER_NAME'], 'b.renania.ro')) {
			$this->data['header_B2B_tab'] = TRUE;
		}
