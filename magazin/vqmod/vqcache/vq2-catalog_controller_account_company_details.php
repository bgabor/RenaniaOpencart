...
class ControllerAccountCompanyDetails extends Controller
{
    ...
    public function update()
    {
        ...
        if( ($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm() )
        {
            ...
            if( $this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4 )
            {
                ...
                $this->model_account_customer->updateInvoiceEmail( $customer_ax_code, $this->request->post['invoice_email'] );
            }
...
//        {
//            $this->data['breadcrumbs'][] = array(
//                'text' => $this->language->get( 'text_company_details' ),
//                'href' => $this->url->link( 'account/company_details/insert', '', 'SSL' ),
//                'separator' => $this->language->get( 'text_separator' )
//            );
//        }
//        else
//        {
            $this->data['breadcrumbs'][] = array(
                'text' => $this->language->get( 'text_company_details' ),
                'href' => $this->url->link( 'account/company_details/update', 'address_id='.$this->request->get['address_id'], 'SSL' ),
                'separator' => $this->language->get( 'text_separator' )
            );
//        }
        ...
        if( $this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4 )
        {
            ...
            $this->load->model('account/customer');
            $customer_ax_code = $this->customer->getAxCode();
            $this->data['nivel_payment_term'] = (int)$this->model_account_customer->returnDiscoutFromAxCustomerTable( $customer_ax_code );
            ...
            $this->data['text_invoice_email_address'] = $this->language->get( 'text_invoice_email_address' );
            
            $invoice_email = $this->model_account_customer->getInvoiceEmail( $customer_ax_code );
                                    
            if( isset( $this->request->post['invoice_email'] ) )
            {
                $this->data['invoice_email'] = $this->request->post['invoice_email'];
            }
            elseif( !empty( $invoice_email ) )
            {
                $this->data['invoice_email'] = $invoice_email;
            }
            else
            {
                $this->data['invoice_email'] = '';
            }
        }
        ...
        if( isset( $this->error['invoice_email'] ) )
        {
            $this->data['error_invoice_email'] = $this->error['invoice_email'];
        }
        else
        {
            $this->data['error_invoice_email'] = '';
        }
        ...
        else
        {
            ...
            $this->data['action'] = $this->url->link( 'account/company_details/update', 'address_id='.$this->request->get['address_id'], 'SSL' );
        }
        ...
        if( file_exists( DIR_TEMPLATE.$this->config->get( 'config_template' ).'/template/account/company_details_form.tpl' ) )
        {
            $this->template = $this->config->get( 'config_template' ).'/template/account/company_details_form.tpl';
        }
        else
        {
            $this->template = 'default/template/account/company_details_form.tpl';
        }
    ...
    protected function validateForm()
    {
        ...
        if ((utf8_strlen($this->request->post['invoice_email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['invoice_email'])) {
      		$this->error['invoice_email'] = $this->language->get('error_invoice_email');
    	  }
