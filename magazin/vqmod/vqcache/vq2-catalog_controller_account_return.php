...
class ControllerAccountReturn extends Controller { 
	...
	public function insert() {
		...
		if (isset($this->error['location'])) {
			$this->data['error_text_location'] = $this->error['location'];
		} else {
			$this->data['error_text_location'] = '';
		}
		...
		if ($this->config->get('config_return_id')) {
			...
			if ($information_info) {
				...
				//$this->data['text_agree'] = sprintf('Am citit si acceptat <a class="colorbox" href="%s" alt="%s"><b>Conditiile de retur</b></a>', $this->url->link('information/information/info', 'information_id=' . $this->config->get('config_return_id'), 'SSL'), $information_info['title'], $information_info['title']);
				$link = str_replace("index.php?route=", '', $this->url->link('conditiile-de-retur.html', '', 'SSL'));
				$this->data['text_agree'] = 'Am citit si acceptat <a class="colorbox" href="'.$link.'" alt="Conditiile de retur"><b>Conditiile de retur</b></a>';
			} else {
  	...
  	protected function validate() {
        ...
        if ((utf8_strlen($this->request->post['location']) < 7) || (utf8_strlen($this->request->post['location']) > 250)) {
      		$this->error['location'] = $this->language->get('error_location');
    	}

    	// here was the "product" validation BEGIN
