...
class ModelAccountOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			if ($order_query->row['cart_id'])
			{
				$support_query = "SELECT
										  csm.*,
										  c.`firstname`,
										  c.`lastname`,
										...
										WHERE `csm`.cart_id = ".$order_query->row['cart_id'].";";

				$support_query = $this->db->query($support_query)->rows;
			}
			else
			{
				$support_query = "";
