...
//            $this->deleteLoginSession();
        ...
        if ( $_SERVER['SERVER_NAME'] == substr(HTTP_SERVER, 8, -1) || $_SERVER['SERVER_NAME'] == substr(HTTPS_SERVER, 8, -1) || strpos($_SERVER['SERVER_NAME'], 'akata') )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if($this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4) {
                        ...
                        $this->redirect( $this->url->link( 'account/account/index', 'type=B2B', 'SSL' ) );
                    } else { //  If normal user logged in from b2b.renania.ro , redirected to magazin.renania.ro
                        $link = str_replace('b2b', 'magazin', $this->url->link( 'account/account/index', 'type=B2C', 'SSL' ));
...
$this->data['register'] = str_replace('b2b.renania.ro','magazin.renania.ro',$this->data['register']);
...
//                $msg .= '</a>';
    ...
    protected function sendValidationCode()
    {
        ...
        $this->db->query("INSERT INTO oc_customer_validation_code SET customer_id = " . $this->session->data['customer_id'] . ", email = '" . $this->customer->getEmail() . "', validation_code = " . $validationcode . ", added_at = NOW(), phone = '".$this->customer->getMobilePhone()."';");
        ...
        if($_SERVER['REMOTE_ADDR'] == "5.2.202.87" || $_SERVER['REMOTE_ADDR'] == "5.15.59.107") {
            $mail->setText(html_entity_decode( $message. " ".$validationcode, ENT_QUOTES, 'UTF-8'));
            $mail->setSubject(html_entity_decode( $subject , ENT_QUOTES, 'UTF-8'));
