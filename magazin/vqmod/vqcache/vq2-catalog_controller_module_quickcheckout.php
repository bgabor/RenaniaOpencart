...
/**
 * Class ControllerModuleQuickcheckout
 *
 * @property    ModelAccountCustomerApiMessages $model_account_customer_api_messages
 */
...
// balazs
//$this->checkout['confirm']['requisition_number'] = '';
//$this->checkout['confirm']['reference'] = '';
// eo balazs
    ...
    {        
        ...
        if( $this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4 ) // B2B customer
        {
            $settings = $this->config->get( 'quickcheckout_settings_b2b' );
            
            if( empty( $settings ) )
            {
                $this->config->load( 'quickcheckout_settings_b2b' );
                
                $settings = $this->config->get( 'quickcheckout_settings_b2b' );
            } 
            
            //load settings from database
            $result = $this->model_setting_setting->getSetting( 'quickcheckout_b2b', $this->config->get( 'config_store_id' ) );
            $this->settings = $result['quickcheckout_b2b'];
        ...
        if( $this->customer->isLogged() )
        {
            ...
            if( $this->checkout['payment_address']['exists'] )
            {
                ...
                if( is_array( $country_data ) )
                    $this->checkout['payment_address'] = array_merge( $this->checkout['payment_address'], $country_data );
            }else
            {
                // if address new
                $country_data = $this->get_country_data( $this->checkout['payment_address']['country_id'], $this->checkout['payment_address']['zone_id'] );
        ...
        $payment_address = array(
            ...
            // balazs
            'middlename' => (isset( $this->checkout['payment_address']['middlename'] )) ? $this->checkout['payment_address']['middlename'] : '',
            ...
            //balazs
            'banca' => (isset( $this->checkout['payment_address']['banca'] )) ? $this->checkout['payment_address']['banca'] : '',
            'iban' => (isset( $this->checkout['payment_address']['iban'] )) ? $this->checkout['payment_address']['iban'] : '',
        ...
        if( is_array( $country_data ) )
            $payment_address = array_merge( $payment_address, $country_data );

        if( isset( $this->checkout['payment_address'] ) )
        {
            if( is_array( $payment_address ) )
                $this->checkout['payment_address'] = array_merge( $this->checkout['payment_address'], $payment_address );
        }else
        {
        ...
        else
        {
            ...
            if( $this->customer->isLogged() )
            {
                ...
                if( $this->checkout['shipping_address']['exists'] )
                {
                    ...
                    if( is_array( $country_data ) )
                        $this->checkout['shipping_address'] = array_merge( $this->checkout['shipping_address'], $country_data );
                }else
                {
                    // if address new
                    $country_data = $this->get_country_data( $this->checkout['shipping_address']['country_id'], $this->checkout['shipping_address']['zone_id'] );
            ...
            if( is_array( $country_data ) )
                $shipping_address = array_merge( $shipping_address, $country_data );
        ...
        {
            if( empty( $this->session->data["login_auto_with_hash"] ) )
            {
                $first = current( $this->checkout['payment_methods'] );
                $default_payment_method = (isset( $this->checkout['payment_methods'][$this->settings['step']['payment_method']['default_option']] )) ?
                                            $this->checkout['payment_methods'][$this->settings['step']['payment_method']['default_option']]
                                                :
                                            $first;
            }
            // If we are logged in with auto login, use the bank transfer in default
            else
            {
                $default_payment_method = null;
                foreach( $this->checkout['payment_methods'] as $payment_method )
                {
                    if( $payment_method["code"] == "bank_transfer" )
                    {
    ...
    private function get_shipping_address_view()
    {
        ...
        $this->data['data'] = $this->array_merge_recursive_distinct( $this->data['data'], $lang );
          ...
          // Attila  quick checkout get addresses if logged user is b2b
        if( $this->customer->isLogged() && ( $this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4 ) ){
    ...
    private function get_payment_method_view()
    {
        ...
        // If user auto logged with hash, payment method will be transfer bancar
        if( isset( $this->session->data['login_auto_with_hash'] ) )
        {
            $this->data['code'] = "bank_transfer";
        }
        // Else check it
...
//        //print ( $query->row['value'] );
//        print_r( unserialize( $query->row['value'] ));
//        die();
        ...
        */
        ...
        {            
            ...
            /*if($_SERVER['REMOTE_ADDR'] == '5.2.202.87' ){
                ...
                var_dump($this->session->data['payment_method']['code']);
                die(' dfabfbsbfd controller/module/quickcheckouit');
            }*/
            ...
            if ($this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4) {
                $this->data['b2b'] = $this->customer->getCustomerGroupId();
            } else {
                $this->data['b2b'] = false;
            }
    ...
    public function confirm_order()
    {
        ...
        elseif( isset( $this->checkout['payment_address'] ) )
        {
            ...
            //balazs
            $data['middlename'] = isset( $this->checkout['payment_address']['middlename'] ) ? $this->checkout['payment_address']['middlename'] : "";
        ...
        //balazs
        $data['payment_banca'] = isset( $payment_address['banca'] ) ? $payment_address['banca'] : "";
        $data['payment_iban'] = isset( $payment_address['iban'] ) ? $payment_address['iban'] : "";
        ...
        $data['comment'] = $this->checkout['confirm']['comment'];

        if( $this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4 )
        {
            $data['requisition_number'] = $this->checkout['confirm']['requisition_number'];
            $data['reference'] = $this->checkout['confirm']['reference'];
        }
        else
        {
...
//            }
//
//            if( in_array( $customer_group_id, array( 2, 4 ) ) ) // gallery, gallery & B2B customer
//            {
//                $this->load->model( 'quickcheckout/ax_order' );
//                $this->model_quickcheckout_ax_order->updateAxOrder( $this->checkout['ax_order_id'], $this->checkout['order_id'], $data );
//            }
//            // end ax
    ...
    private function createAndSendCXMLToAutoLoggedInCustomer( $order_data )
    {
        if( ! $this->session->data["login_auto_with_hash"] )
        {
            return;
        }

        $tax_value = 0;
        foreach( $order_data["totals"] as $total_data )
        {
            if( $total_data["code"] == "tax" )
            {
                $tax_value == $total_data["value"];
                break;
            }
        }
        ...
        $xml = '<?xml version="1.0" encoding="UTF-8"?>
                <!DOCTYPE cXML SYSTEM "http://xml.cXML.org/schemas/cXML/1.1.010/cXML.dtd">
                    <cXML version="1.1.010" payloadID="1413878653.631907543@YOURDOMAIN" timestamp="2014-10-21T10:04:13+02:00">
	                    <Header>
		                    <From>
                                <Credential domain="'.$this->request->server["HTTP_HOST"].'">
                                    <Identity>Renania</Identity>
                                </Credential>
                            </From>
                            <To>
                                <Credential domain="takata">
                                    <Identity>takata</Identity>
                                </Credential>
                            </To>
                            <Sender>
                                <Credential domain="'.$this->request->server["HTTP_HOST"].'">
                                ...
                                <UserAgent>Renania-Software WEB-Interface</UserAgent>
                            </Sender>
                        </Header>
                        <Message>
                            <inReplyTo>20141021103.17208@IPADDRESS</inReplyTo>
                            <PunchOutOrderMessage>
                                <BuyerCookie>2103194819</BuyerCookie>
                                <PunchOutOrderMessageHeader operationAllowed="edit">
                                    <Total>
                                        <Money currency="'.$order_data["currency_code"].'">'.$order_data["total"].'</Money>
                                    </Total>
                                    <Tax>
                                        <Money currency="'.$order_data["currency_code"].'">'.$tax_value.'</Money>
                                    </Tax>
                        ...
                        foreach( $order_data["products"] as $product )
                        {
                            $xml .= '
                                    <ItemID>
                                        <SupplierPartID>'.$product["product_id"].'</SupplierPartID>
                                        <SupplierPartAuxiliaryID>'.$product["product_id"].'</SupplierPartAuxiliaryID>
                                    </ItemID>
                                    <ItemDetail>
                                        <UnitPrice>
                                            <Money currency="'.$order_data["currency_code"].'">'
                                                .$product["price"]
                                            .'</Money>
                                        </UnitPrice>
        ...
        */
        ...
        $this->sendXmlToUser( $order_data );
    }

    /*
    private function saveApiMessage( & $order_data )
    {
        $file_path = DIR_SYSTEM."../xmls/";
        if( ! file_exists( $file_path ) )
        {
            mkdir( $file_path, 0755, TRUE );
        }

        $file_name = "order_xml_".$this->checkout['order_id']."_".date( "Ymd-His" ).".cxml";

        file_put_contents( $file_path.$file_name, $xml, FILE_APPEND );

        $this->sendXmlToUser( $xml, $order_data );
    }
    */

    private function sendXmlToUser( & $order_data )
    {
        if( empty( $this->session->data["login_auto_details"] ) )
        {
            return;
        }

        $customer_auto_login = $this->session->data["login_auto_details"];

        unset( $this->session->data["show_xml_for_auto_login"]);
        unset( $this->session->data["xml_data_for_auto_login"]);

        $api_push_url = $customer_auto_login["api_push_url"];
        if( empty( $api_push_url ) )
        {
            $this->session->data["show_xml_for_auto_login"] = 1;
            $this->session->data["xml_data_for_auto_login"] = $order_data;
            return;
        }
        ...
        /*
        $post_data = 'cXML-urlencoded='.htmlentities( $xml );

        $curl = curl_init( $api_push_url );
        curl_setopt( $curl, CURLOPT_RETURNTRANSFER, TRUE );
        curl_setopt( $curl, CURLOPT_POST, TRUE );
        curl_setopt( $curl, CURLOPT_POSTFIELDS, $post_data );

        $message_received = curl_exec( $curl );
        curl_close( $curl );

        $this->load->model( 'account/customer_api_messages' );

        $data = array(
            "customer_id"       => $customer_auto_login["customer_id"],
            "push_url"          => $customer_auto_login["api_push_url"],
            "message_sent"      => $post_data,
    ...
    function update_order()
    {
        ...
        if( $this->cart->hasShipping() )
        {
            ...
            // Attila begin if logged user is b2b than shipping data come from B2B_adresa
            /*if ( $this->customer->isLogged() ) {
                $this->load->model( 'account/customer' );
                $customer_info = $this->model_account_customer->getCustomer( $this->session->data['customer_id'] );
                ...
                //echo "<pre>"; print_r($data); die('sdfb');
            } else {*/
    ...
    private function get_total_data( &$total_data, &$total, &$taxes )
    {
        ...
        /*if($_SERVER['REMOTE_ADDR'] == '5.2.202.87') {
            echo "<pre>"; print_r($total_data); die('sdvsdv');
        }*/
    ...
    private function get_shipping_methods( $shipping_address )
    {
        ...
        foreach( $results as $result )
        {
            if( $this->config->get( $result['code'].'_status' ) )
            {
                ...
                // daca subtotalul este mai mare decat cel specificat pe partea de admin atunci numai "Livrare gratuita" va aparea
                if( $quote )
                {                    
