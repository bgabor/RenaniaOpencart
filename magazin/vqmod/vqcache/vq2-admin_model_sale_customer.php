...
class ModelSaleCustomer extends Model {
      	...
      	$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "',  
            middlename = '" . $this->db->escape($data['middlename']) . "', lastname = '" . $this->db->escape($data['lastname']) . "',identity_card_number = '" . $this->db->escape($data['identity_card_number']) . "',mobile_phone = '" . $this->db->escape($data['mobile_phone']) . "',
             email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', date_added = NOW(), ax_code = '" . $this->db->escape($data['ax_code']) . "' ");//, payment_term = '" . $this->db->escape($data['payment_term']) . "'
	...
	public function editCustomer($customer_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "',  
            middlename = '" . $this->db->escape($data['middlename']) . "', lastname = '" . $this->db->escape($data['lastname']) . "',identity_card_number = '" . $this->db->escape($data['identity_card_number']) . "',mobile_phone = '" . $this->db->escape($data['mobile_phone']) . "',
             email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', status = '" . (int)$data['status'] . "', black_list = " . (int)$data['black_list'] . "  WHERE customer_id = '" . (int)$customer_id . "'");

        if ( (int)$data['customer_group_id'] == 3 || (int)$data['customer_group_id'] == 4 )
        {
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET ax_code = '" . $this->db->escape($data['ax_code']) . "',  permission = '" . $this->db->escape($data['permission']) . "'  WHERE customer_id = '" . (int)$customer_id . "'");// payment_term = '" . $this->db->escape($data['payment_term']) . "',
        }
        else
        {
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET ax_code = '',  permission='' WHERE customer_id = '" . (int)$customer_id . "'");
            //payment_term = '',
        }
  ...
  public function getB2bAddresses($ax_code) {
		$address_data = array();
