...
<!--    <div class="display"><b><?php echo $text_display; ?></b> <?php echo $text_list; ?> <b>/</b> <a onclick="display('grid');"><?php echo $text_grid; ?></a></div>-->
  ...
  <div class="product-list">
    ...
    <div>
      ...
      <?php } ?>
      <div class="cart" id="cart_<?php echo $product['product_id']; ?>">
        <?php if ($this->customer->isLogged()) { ?>
...
<!--      <div class="wishlist"><a onclick="addToWishList('<?php echo $product['product_id']; ?>');"><?php echo $button_wishlist; ?></a></div>-->
      ...
      <div class="product_id"><?php echo $product['product_id']; ?></div>
      <div class="product_option_quantity"><?php echo $product['product_option_quantity']; ?></div>
      <div class="button_cart"><?php echo $button_cart; ?></div>
...
function display(view) {
	...
	if (view == 'list') {
		...
		$('.product-list > div').each(function(index, element) {
         ...
         var product_option_quantity = $(element).find('.product_option_quantity').html();
         var product_id = $(element).find('.product_id').html();
         var button_cart = $(element).find('.button_cart').html(); 
         
			html  = '<div class="right">';
			html += '  <div class="cart" id="'+ product_id +'">' + $(element).find('.cart').html() + '</div>';
//			html += '  <div class="wishlist">' + $(element).find('.wishlist').html() + '</div>';
         ...
         {
            $("#"+product_id).html( '<a onclick="addToCart(' + product_id + ');" class="button">'+ button_cart + '</a>' );
         }
		...
		$('.product-grid > div').each(function(index, element) {
         ...
         if ( product_option_quantity == 0 )
         {
            html += '<a onclick="addToCart(' + product_id + ');" class="button">'+ button_cart + '</a>';
         }
         else
         {
            html += '<div class="cart" id="cart_'+ product_id +'">' + $(element).find('.cart').html() + '</div>';
         }
         
//			html += '<div class="wishlist">' + $(element).find('.wishlist').html() + '</div>';
			html += '<div class="compare">' + $(element).find('.compare').html() + '</div>';
         
			html += '<div class="product_id" style="display:none;">' + product_id + '</div>';
			html += '<div class="product_option_quantity" style="display:none;">' + product_option_quantity + '</div>';
			html += '<div class="button_cart" style="display:none;">' + button_cart + '</div>';      
