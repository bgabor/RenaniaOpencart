      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <div id="tab-general">
          <div id="vtabs" class="vtabs"><a href="#tab-customer"><?php echo $tab_general; ?></a>
            ...
            <?php $address_row++; ?>
            <?php } ?>
            <?php if($customer_group_id == 3 || $customer_group_id == 4) { ?>
             </div>
             <?php } else { ?>
          ...
          <div id="tab-customer" class="vtabs-content">
            <table class="form">
              ...
              <tr class="company-ax-code-display">
                <td><?php echo $entry_ax_code; ?></td>
                <td><input type="text" name="ax_code" value="<?php echo $ax_code; ?>" /></td>
              </tr>
              <tr class="customer-type-display">
                <td><?php echo $entry_customer_type; ?></td>
                <td>
                   <select name="permission">
                    <?php if ($permission == $text_full_permision ) { ?>
                    <option value="<?php echo $text_full_permision; ?>" selected="selected"><?php echo $text_master; ?></option>
                    <option value="<?php echo $text_limited_permision; ?>"><?php echo $text_slave; ?></option>
                    <?php } else { ?>
                    <option value="<?php echo $text_full_permision; ?>"><?php echo $text_master; ?></option>
                    <option value="<?php echo $text_limited_permision; ?>" selected="selected"><?php echo $text_slave; ?></option>
                    <?php } ?>
                  </select>
                </td>
              </tr>
<!--              <tr class="company-payment-term-display">
                <td><?php echo $entry_payment_term; ?></td>
                <td><input type="text" name="payment_term" value="<?php echo $payment_term; ?>" /></td>
              </tr>-->
...
<script type="text/javascript"><!--
   ...
   <?php if ($ax_code) { ?>
      $('.company-ax-code-display').show();
   <?php }
   else{
   ?>
      $('.company-ax-code-display').hide();
      <?php } ?>
      
   
   <?php if ( $permission && ( $customer_group_id == 3 || $customer_group_id == 4 ) ) { ?>
      $('.customer-type-display').show();
   <?php }
   else{
   ?>
      $('.customer-type-display').hide();
   <?php } ?>
   
  
   
   /*<?php if (!empty($payment_term) && $payment_term != '0000-00-00' ) { ?>
      $('.company-payment-term-display').show();
   <?php }
   else{
   ?>
      $('.company-payment-term-display').hide();
      <?php } ?>
      */
   });
            
   $('select[name=\'customer_group_id\']').change(function (e){   
      if ( $('select[name=\'customer_group_id\']').val() == 3 ||  $('select[name=\'customer_group_id\']').val() == 4 )
      {
         $('.company-ax-code-display').show();
         $('.company-payment-term-display').show();
         $('.customer-type-display').show();
