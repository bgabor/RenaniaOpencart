...
class ModelSettingStore extends Model {
	...
	public function getTotalStoresByOrderStatusId($order_status_id) {
 	...
 	public function getTotalStoresByDocumentId($document_id) {
    $account_query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "setting WHERE `key` = 'config_account_id' AND `value` = '" . (int)$document_id . "' AND store_id != '0'");
      	
		$checkout_query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "setting WHERE `key` = 'config_checkout_id' AND `value` = '" . (int)$document_id . "' AND store_id != '0'");
