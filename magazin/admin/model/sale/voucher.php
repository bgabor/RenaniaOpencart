...
class ModelSaleVoucher extends Model
{
    ...
    {
        $this->db->query( "INSERT INTO ".DB_PREFIX."voucher SET order_id = '".( int ) $data['order_id']."',code = '".$this->db->escape( $data['code'] )."', from_name = '".$this->db->escape( $data['from_name'] )."', from_email = '".$this->db->escape( $data['from_email'] )."', to_name = '".$this->db->escape( $data['to_name'] )."', to_email = '".$this->db->escape( $data['to_email'] )."', voucher_theme_id = '".( int ) $data['voucher_theme_id']."', message = '".$this->db->escape( $data['message'] )."', amount = '".( float ) $data['amount']."', status = '".( int ) $data['status']."', date_added = NOW(), date_expiration = '".$data['date_expiration']."', set_voucher = '".$data['set_voucher']."' " );
...
//    public function getOrderId( $order_id )
    public function getSetVoucher( $order_id )
    {
        $query = $this->db->query( "SELECT COUNT(voucher_id) as sum FROM ".DB_PREFIX."voucher WHERE set_voucher = '".( int ) $order_id."'" );
