...
class ModelNeNewsletter extends Model {
    ...
    public function send($data) {
        ...
        $message .= '<body style="padding:0;margin:0;">' .
            html_entity_decode($data['message'], ENT_COMPAT, 'UTF-8') . '</body>' . PHP_EOL;
        ...
        if (is_dir(dirname(DIR_DOWNLOAD) . DIRECTORY_SEPARATOR . 'attachments' .
                    DIRECTORY_SEPARATOR . $data['attachments_id'])) {
            foreach (glob(dirname(DIR_DOWNLOAD) . DIRECTORY_SEPARATOR . 'attachments' .
                DIRECTORY_SEPARATOR . $data['attachments_id'] . DIRECTORY_SEPARATOR . '*') as $path) {
                if (file_exists($path)) {
                    $attachments[] = array(
        ...
        foreach ($data['emails'] as $email => $info) {
            ...
            if ($this->config->get('ne_throttle') && $newsletter_id) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "ne_queue SET email = '" . $this->db->escape($email) . "', firstname = '" . $this->db->escape($info['firstname']) . "', lastname = '" . $this->db->escape($info['lastname']) . "', history_id = '" . (int)$newsletter_id . "'");
                continue;
            }
    ...
    public function addHistoryQueue($newsletter_id, $data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "ne_stats SET history_id = '" . (int)$newsletter_id . "', queue = '" . (int)$data['queue'] . "', recipients = '" . (int)$data['recipients'] . "', views = '0'");
    }
