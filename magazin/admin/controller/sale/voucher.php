...
class ControllerSaleVoucher extends Controller
{
    ...
    public function insert()
    {
        ...
        {
            //$this->request->post['date_expiration'] = date( "Y-m-d H:i:s", mktime( date( 'H' ), date( 'i' ), date( 's' ), date( 'm' ), date( 'd' ) + 30, date( 'Y' ) ) );
    ...
    public function addVoucher()
    {
        $data = array( );
        $data['order_id'] = 0; 
        $data['code'] = substr( md5( uniqid( rand(), true ) ), 0, 6 );
        $data['from_name'] = 'Renania Trade';
        $data['from_email'] = 'claudiu.baias@renania.ro';
        ...
        if( $order_info )
        {
            ...
            $data['voucher_theme_id'] = 8; //general
            $data['message'] = 'message';
            
            $amount = ( $order_info['total'] * 20 ) / 100 ;
            $data['amount'] = $amount;
            $data['status'] = 1;
            $data['date_added'] = date( 'Y-m-d H:i:s' );
            //$data['date_expiration'] = date( "Y-m-d H:i", mktime( date( 'H' ), date( 'i' ), date( 's' ), date( 'm' ), date( 'd' ) + 30, date( 'Y' ) ) );
            $data['set_voucher'] = $this->request->get['order_id'];
        }
        $this->load->model( 'sale/voucher' );
        $new_voucher_id = $this->model_sale_voucher->addVoucher( $data );

        $this->language->load( 'sale/voucher' );
        
        
        // send mail for client
        $this->model_sale_voucher->sendVoucher( $new_voucher_id );
        
        $response = array();
        $response['message'] = $this->language->get( 'text_addsuccess' ); 
        $response['already_have_voucher'] = $this->model_sale_voucher->getSetVoucher( $this->request->get['order_id'] ); 
