...
Journal.setupMenu = function (type) {
    ...
    if (type === 'mobile') {
        ...
        $('#sort_heading').toggle(function () {
            console.log('tyype '+type);
            $('#sort_heading').css('background','#ddd');
            $(this).next('ul').stop(true, true).slideDown(250);
        }, function () {
            $(this).next('ul').stop(true, true).slideUp(150);
        });

        $('#sort_heading').on("click  touchstart", function(){
            console.log('biind ');
            $('#sort_heading').css('background','#ddd');
        });
        ...
        $('#filter_btn').toggle(function () {
            if ($(window).width() < 760) {
                $('.product-filter').stop(true, true).slideDown(250);
            }
        }, function () {
            if ($(window).width() < 760) {
                $('.product-filter').stop(true, true).slideUp(150);
            }
        });

        $('#limit_heading').toggle(function () {
            $(this).next('ul').stop(true, true).slideDown(250);
        }, function () {
            $(this).next('ul').stop(true, true).slideUp(150);
        });
...
Journal.searchAutoComplete = function () {
    $('#search input').autocomplete2({
        appendTo: '.search_container #search',
        width: '100%',
...
Journal.onDesktop = function () {
    ...
    $('#filter_btn').live('click', function () {
        ...
        if ($(".product-filter").is(':hidden')) {
            //console.log('cliick s filter_btn');
            //$('#filter_btn').css('background','#eee');
            $('.product-filter').addClass('active');
            $('#filter_btn').find('div').removeClass('down_arrow');
            $('.product-filter').fadeIn();

        } else {

            //$('#filter_btn').css('background','#fff');
            $(this).removeClass('active');
            $('#filter_btn').find('div').addClass('down_arrow');
            $('.product-filter').fadeOut();

        };
    });

    $('#sort_heading').live('click', function () {

        if (!$('.sort > ul').hasClass('visible')) {
            $('.sort > ul').addClass('visible');
            //console.log('cliick s filter_btn');
            //$('#sort_heading').css('background','#eee');
            $('.sort > ul').fadeIn();

        } else {
            $('.sort > ul').removeClass('visible');
            //console.log('cliick h filter_btn');
            //$('#sort_heading').css('background','#fff');
            $('.sort > ul').fadeOut();

        };
    });

    $('#sort_heading').live('mouseover', function () {
    ...
    $('#search_category_heading').live('mouseover', function () {
        if ($('#search_category_sort > ul').is(':hidden')) {
            $('#search_category_sort > ul').fadeIn();
        }
    });

    $('#search_category_sort').live('mouseleave', function () {
        if ($('#search_category_sort > ul').is(':visible')) {
            $('#search_category_sort > ul').fadeOut();
        }
    });

    $('#limit_heading').live('mouseover', function () {
        if ($('.limit > ul').is(':hidden')) {
            $('.limit > ul').fadeIn();
        }
    });

    $('.limit').live('mouseleave', function () {
        if ($('.limit > ul').is(':visible')) {
            $('.limit > ul').fadeOut();
        }
    });

    $('#top-bar-product-category').live('mouseover', function () {
        if (!$("#top-bar-product-category").hasClass('active')) {
            $('#top-bar-product-category').addClass('active');
            $('#top-bar-product-category').find('.content').show();
            //$('#top-bar-product-category').find('.content').css('opacity','1');
        }
        $('#top-bar-product-category').live('mouseleave', function () {
            $(this).removeClass('active');
            $('#top-bar-product-category').find('.content').hide();
        });
    });

    $('#search_category_sort > ul > li > a').on('click', function() {
        //$('#category_hidden_select > #'+$(this).attr('id')).prop('selected', true);
        $("#category_hidden_select").val($(this).attr('id'));
        $("#category_hidden_select").change();
    ...
    if ($('header').hasClass('journal-header-center')) {
        ...
        $(window).scroll(function () {
            ...
            if ($(window).scrollTop() > 55) {
                $('.breadcrumb').addClass('fixed_breadcrumbs');
                //console.log('add sccroll '+$(window).scrollTop());
            }
            if ($(window).scrollTop() < 55) {
                $('.breadcrumb').removeClass('fixed_breadcrumbs');
                //console.log('remoove sccroll '+$(window).scrollTop());
            }
        });
        return;
    }
...
Journal.SuperFilter.filter = function ($parent) {
    ...
    if ($('.sort').length > 0) {
        ...
        //var value = $('.sort ul li a.selected').attr('id').split('sort=')[1].split('&');
        url_parts.push('sort=' + value[0]);
        url_parts.push('order=' + value[1].replace('order=', ''));
        console.log('uurl_parts'+url_parts);
    }

    /* add limit */
    if ($('.limit').length > 0) {
        console.log('jornal '+$('.limit select option:selected').text());
    ...
    $.each(url.split('/'), function (index, part) {
        ...
        if (attributePattern.test(part)) {
            ...
            $.each(values[2].split(','), function (i, value) {
                console.log('.sf-attribute-' + values[1] + ' input[value="' + value + '"]');
                //$parent.find('.sf-attribute-' + values[1] + ' input[value="' + value + '"]').attr('checked', 'checked');
                $parent.find('.sf-attribute-' + values[1]).attr('checked', 'checked');
            });
            return;
        }
...
$(document).ready(function(){
    ...
    $('.sort ul li a').on('click', function(){
        ...
        $('.sort > select').val(selected_sort).trigger('change');
    ...
    $('.limit ul li a').on('click', function(){

        var selected_limit = $(this).attr('id');

        $('.limit ul li a').each(function() {
            $(this).removeClass('selected');
        });
        $(this).addClass('selected');

        $('.limit > select').val(selected_limit).trigger('change');
        console.log('limitt cli '+selected_limit);

        $(".limit > select > option").each(function() {
            $(this).removeAttr('selected');
        });
        $('.limit > select > option[value$="'+selected_limit+'"]').prop('selected', true);
    });

    $( window ).scroll(function() {
        //console.log('scrool');
        if ($(window).scrollTop() > 10 && $(window).width() < 760) {
            $('.journal-logo').hide();
            //console.log('journal-logo hidee');
        } else {
            $('.journal-logo').show();
            //console.log('journal-logo showw');
        }

        if ($(window).width() > 760 && $(window).width() < 1400 && !$('.breadcrumb').hasClass('fixed_breadcrumbs')) {
            if ($(window).scrollTop() > 10) {
                //console.log('adsmaller_logo');
                $('#logo > a').addClass('smaller_logo');
            } else {
                //console.log('reemsmaller_logo');
                $('#logo > a').removeClass('smaller_logo');
            }
        }

        if ($(window).width() < 1000) {
            if ($(window).scrollTop() > 10) {
                //console.log('2addd smaller_logo');
                $('#logo > a').addClass('smaller_logo');
            } else {
                //console.log('2reemove smaller_logo');
                $('#logo > a').removeClass('smaller_logo');
            }
        }
