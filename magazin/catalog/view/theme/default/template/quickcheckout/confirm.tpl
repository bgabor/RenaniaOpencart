  ...
  <div class="box">
    ...
    <div class="box-content">
      <div id="confirm_inputs">
        ...
        foreach($confirm['fields'] as $field){
    	if(isset($field['type'])) {
    ...
    <div id="<?php echo $field['id']; ?>_input" class="box box-border sort-item <?php echo $field['id']; ?> <?php echo ($field['class'])? $field['class'] : ''; ?>" sort-data="<?php echo $field['sort_order']; ?>">
      ...
      <div class="box-content">
        ...
        <?php
    		...
    		case "checkbox":
		?>
        <div id="<?php echo $field['id']; ?>_input" class="checkbox-input sort-item <?php echo (!$field['display'])? 'hide' : ''; ?> <?php echo ($field['class'])? $field['class'] : ''; ?>" sort-data="<?php echo $field['sort_order']; ?>" style="<?php if ( strpos($_SERVER['SERVER_NAME'], 'akata') || $_SERVER['SERVER_NAME'] == substr(B2B_HTTP_SERVER, 7, -1) || $_SERVER['SERVER_NAME'] == substr(B2B_HTTPS_SERVER, 8, -1) ) echo "display:none"; ?>">
          <?php if($field['value'] == 1 || strpos($_SERVER['SERVER_NAME'], 'akata') || $_SERVER['SERVER_NAME'] == substr(B2B_HTTP_SERVER, 7, -1) || $_SERVER['SERVER_NAME'] == substr(B2B_HTTPS_SERVER, 8, -1) ){?>
        ...
        <div id="<?php echo $field['id']; ?>_input" class="text-input sort-item <?php echo (!$field['display'])? 'hide' : ''; ?> <?php echo ($field['class'])? $field['class'] : ''; ?>" sort-data="<?php echo $field['sort_order']; ?>">
          <label for="confirm_<?php echo $field['id']; ?>"> <span class="required <?php echo (!isset($field['require']) ||  !$field['require']) ? 'hide' : ''; ?>">*</span> <span class="text"><?php echo $field['title']; ?></span> </label>
<!--          <textarea name="confirm[<?php echo $field['id']; ?>]" id="confirm_<?php echo $field['id']; ?>" data-require="<?php echo (isset($field['require']) && $field['require']) ? 'require' : ''; ?>" data-refresh="<?php echo ($field['refresh']) ? $field['refresh'] : 0; ?>"><?php echo isset($field['value'])? $field['value'] : ''; ?></textarea>-->
          <input type="text" value="<?php echo isset($field['value'])? $field['value'] : ''; ?>" name="confirm[<?php echo $field['id']; ?>]" id="confirm_<?php echo $field['id']; ?>"  />
        </div>
      ...
      <div id="payment_input">
        <div id="confirm_payment" style="<?php echo (strpos($_SERVER['SERVER_NAME'], 'akata')) ? "display:none" : "" ?>"><?php echo $payment; ?></div>
        <div class="buttons">
          <div class="right">
            <input type="button" id="confirm_order" class="button btn btn-primary <?php if(isset($payment)){ echo "checkout_order"; }else{ echo "";  } ?>" value="<?php if(isset($payment)){ echo $button_confirm; }else{ echo $button_continue;  } ?>" />
          </div>
        </div>
      </div>
      <div class="clear"></div>
