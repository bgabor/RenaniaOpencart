...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        $('input[name=\'filter_model\']').autocomplete({
            ...
            select: function(event, ui) {
                ...
                console.log('from model insert product id '+ui.item.product_id);
    ...
    $('#add_to_cart_btn').live('click', function() {
        $( ".product_to_order_line" ).each(function( index ) {
    ...
    function add_to_fast_order_list() {
        if($.trim($("#fast_order_list").html())==''){
            ...
            var product_to_order_list_head = '<div id="product_to_order_list_head">';
            product_to_order_list_head += '<div class="product_to_order_name">'+$('.filter_name > p').html()+'</div>';
            product_to_order_list_head += '<div class="product_to_order_model">'+$('.filter_model > p').html()+'</div>';
            product_to_order_list_head += '<div class="product_to_order_stoc">'+$('.filter_stoc > p').html()+'</div>';
            product_to_order_list_head += '<div class="product_to_order_quantity">'+$('.filter_quantity > p').html()+'</div>';
            product_to_order_list_head += '<div class="product_to_order_price">'+$('.filter_price > p').html()+'</div>';
            product_to_order_list_head += '<div class="product_to_order_total_price">'+$('.filter_total_price > p').html()+'</div>';
            product_to_order_list_head += '</div>';
            $('#fast_order_list').append(product_to_order_list_head);
            $('#fast_order_list').append('<div id="fast_order_list_bottom"><a id="add_to_cart_btn" class="button">Adaugare in cos</a><div id="fast_order_list_total"><b>Total - '+$('.filter_total_price > div').html()+'</b></div></div>');
        }
        ...
        var product_to_order = '<div class="product_to_order_line"><div class="product_to_order_name"><h2>'+product_name+'</h2>';
        ...
        product_to_order += '</div>'; // close product_to_order_name div

        product_to_order += '<div class="product_to_order_model">'+product_model+'</div>';

        product_to_order += '<div class="product_to_order_stoc">'+stoc+'</div>';

        product_to_order += '<div class="product_to_order_quantity"><span>'+quantity+'</span><input type="hidden" id="quantity" name="quantity" value="'+quantity+'" ></div>';

        product_to_order += '<div class="product_to_order_price">'+price_text+'</div>';

        product_to_order += '<div class="product_to_order_total_price" data-total="'+total_price+'" data-currency="'+currency+'">'+total_price_text+'</div>';

        product_to_order += '<div class="delete_product"><a class="button delete_product_line">Eliminare</a></div>'

        product_to_order += '</div>';

        $('<hr>'+product_to_order).insertAfter('#product_to_order_list_head');
