	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				if (json['error']['banca']) {
					$('#payment-address input[name=\'banca\'] + br').after('<span class="error">' + json['error']['banca'] + '</span>');
				}	
				if (json['error']['iban']) {
					$('#payment-address input[name=\'iban\'] + br').after('<span class="error">' + json['error']['iban'] + '</span>');
				}	
				/* balazs */														
