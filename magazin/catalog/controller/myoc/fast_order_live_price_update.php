...
class ControllerMyocFastOrderLivePriceUpdate extends Controller
{
    ...
    public function index()
    {
        ...
        if ($this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4) {
            ...
            $finalPrice = $product_info['special'] ? $this->cart->getTotal() : $calculatedPrice;
        }
...
//                        }
                ...
                else
                {
                    ...
                    $price_with_tax = preg_replace('/[^,0-9]/i', "", $taxOfOneProductB2B );// send back without currency symbol
                    $unformat_price_with_tax = preg_replace('/[^,0-9]/i', "", $taxOfOneProductB2B);
                    $total_price_with_tax = $this->currency->format(number_format($taxOfOneProductB2B * $quantity,2));
                    $total_price = number_format($taxOfOneProductB2B * $quantity,2);
                    $json['test'] = (float)$quantity;
                ...
                $taxOfOneProductB2B = $this->tax->calculate( $calculatedPrice, $product_info['tax_class_id'], TRUE );
                $total_price = number_format($taxOfOneProductB2B * $quantity,2);
                $total_price_with_tax = $this->currency->format(number_format($taxOfOneProductB2B * $quantity,2));
            }

            $json['have_b2b_price'] = $have_b2b_price;
            $json['price_with_tax'] = $taxOfOneProductB2B ? $taxOfOneProductB2B : $priceB2B; //$price_with_tax;
            $json['price_and_text'] = ( $B2B ? $pr : $finalValue ); // $finalValue;
            $json['total_price_and_text'] = $total_price_with_tax;
