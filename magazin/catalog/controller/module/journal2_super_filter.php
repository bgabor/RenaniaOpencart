...
class ControllerModuleJournal2SuperFilter extends Controller {
    ...
    public function filters() {
		if (strtolower($this->request->server['REQUEST_METHOD']) !== 'post') return;
    ...
    public function products() {
        ...
        foreach ($results as $result) {
            ...
            $this->data['products'][] = array(
                ...
                'thumb2'      => $image2,
                'name'        => $result['name'],
                'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, Front::$IS_OC2 ? $this->config->get('config_product_description_length') : 100) . '..',
                'price'       => $price,
                'special'     => $special,
                'date_end'    => $date_end,
                'tax'         => $tax,
