...
class ControllerAccountLogin extends Controller
{
    ...
    public function index()
    {
        ...
        if( ($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate() )
        {
			...
			if ( empty($this->session->data['login_validation_code_sent']) && ( $this->customer->getCustomerGroupId() == 3 || $this->customer->getCustomerGroupId() == 4) && !isset($this->request->get['hash']))
			{            
            ...
            {
                if ( $this->request->post['B2B_authentication'] == 1 && !isset($this->request->get['hash']) )
                {
        ...
        elseif (isset($this->request->get['hash']) && !empty($this->request->get['hash'])) {
            $this->redirect( $this->url->link( 'account/account', '', 'SSL' ) );
            /*if($_SERVER['REMOTE_ADDR'] == '5.2.202.87'){
                $isLoggedButNoValidation = $this->customer->isLoggedB2BWithoutValidationCode();
                var_dump($isLoggedButNoValidation);
                var_dump($this->request->get['hash']);
                die('ok 6');
            }*/
        }
        else
    ...
    private function tryToAutoLogin()
    {
        ...
        /*if($_SERVER['REMOTE_ADDR'] == '5.2.202.87'){*/
            $isLoggedButNoValidation = $this->customer->isLoggedB2BWithoutValidationCode();
            if ($isLoggedButNoValidation) {
                $isLoggedButNoValidation = NULL;
            }
            /*var_dump($isLoggedButNoValidation);
            var_dump($this->request->get['hash']);
    ...
    protected function sendValidationCode()
    {
        ...
        $subject =  $this->language->get( 'mail_subject_validation_code' );
            ...
            /*$mail->setText(html_entity_decode( $message. " ".$validationcode, ENT_QUOTES, 'UTF-8'));
            $mail->setSubject(html_entity_decode( $subject , ENT_QUOTES, 'UTF-8'));
            $mail->setTo( "demeterattila90@gmail.com" );
            $mail->send();*/

        if( $_SERVER['REMOTE_ADDR'] == '5.2.202.87' ){
            //echo "<pre>"; print_r($mail); die('1');
        }
    }
