<?php 
class ControllerAccountNov7Order extends Controller {
	private $error = array();
...
//    	}
		...
		$query = $this->db->query("SELECT order_id, firstname, lastname, email, shipping_method, shipping_code, total,ip, date_added FROM oc_order WHERE date_added LIKE '%2014-11-12%';");//2014-11-07
		$orders = array();
		if ($query->num_rows) {
		...
		{
			$query2 = $this->db->query("SELECT * FROM oc_order_product WHERE order_id=".$order['order_id'].";");
			if ($query2->num_rows) {

				foreach ($query2->rows as $result2) {
					$order_products[$result2['order_id']][] = array(
						'name'          => $result2['name'],
						'model'         => $result2['model'],
		...
		foreach( $orders as $order )
		{
			$query3 = $this->db->query("SELECT * FROM oc_order_total WHERE order_id=".$order['order_id']." ");
			if ($query3->num_rows) {

				foreach ($query3->rows as $result3) {
					$order_total[$result3['order_id']][$result3['code'] ] = $result3['value'];
					/*	$result3['code']          => $result3['value']
					);*/
				}
			}
		}
		...
		//SELECT * FROM `oc_order_total` WHERE `order_id` = 399 ORDER BY `order_total_id` DESC
/*		print_r( $this->data['order_total'] );
		die();*/
...
/*		$pagination = new Pagination();
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/nov7_order.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/nov7_order.tpl';
		} else {
			$this->template = 'default/template/account/nov7_order.tpl';
		}
