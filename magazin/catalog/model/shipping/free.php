...
class ModelShippingFree extends Model {
 ...
 //   print $this->cart->getSubTotal();die();
    if ( !in_array( $this->customer->getCustomerGroupId(), array( 3,4 ) )  ) // regular client
    {
    ...
    {
        if ($this->cart->getSubTotal() < $this->config->get('free_total_b2b')) 
        {//die('else');
            $status = false;
        }
    }
