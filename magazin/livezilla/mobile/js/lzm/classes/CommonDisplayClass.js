...
CommonDisplayClass.prototype.createLayout = function(userStatusLogo, appOs) {
    ...
    var profileSelectWidth = Math.min(inputContainerWidth - 75, loginTableFirstColumnWidth + loginTableSecondColumnWidth - 25 - profileButtonWidth);
    ...
    var thisOrientationButtonCss = (this.isApp && appOs != 'ios' && (minWidthHeight >= 520 || this.orientation == 'horizontal')) ?
        {'background-image': this.addBrowserSpecificGradient('', appOs),
        cursor: 'pointer', border: '1px solid #ccc', 'border-radius': '1em',
        width: '29px', height: '29px', display: 'block'} : {'display': 'none'};
    var thisOrientationButtonInnerCss = {'background-image': 'url(\'img/device_' + targetOrientation + '2.png\')', 'background-repeat': 'no-repeat',
        'background-position': 'center', padding: '0px 13px 13px 13px', opacity: 0.4};
    //Add two icons here for the benefit of my release archive creation script
    // img/device_horizontal2.png
    // img/device_vertical2.png
