    ...
    if (runningFromApp) {
        if (myUrl.indexOf('?') != -1) {
            ...
            if (urlParts[1] == 'ERROR') {
                ...
                lzm_commonDialog.createAlertDialog(errorMessage, [{id: 'alert-btn-ok', name: t('Ok')}]);
...
var fillStringsFromTranslation = function(selectedIndex, selectedStatus) {
    if (loopCounter > 49 || lzm_commonTranslation.translationArray.length != 0) {
        ...
        $('#login_btn').text(t('Log in'));
        //$('#configure_btn').text(t('Server Profiles'));
        //$('#user_status-inner-text').text(t(''));
...
var createLoginForm = function(profileIndex, userStatus) {
    var loginFormHtml = '<table class="login-data-table" style="width: 100%;"><tr id="username-container">' +
        '<td style="width: 50px; text-align: right; padding-right: 5px;"><label id="username-text" for="username"></label></td>' +
        '<td><input type="text" data-role="none" name="username" id="username" class="login-input lzm-text-input"' +
        ' placeholder="Username" autocapitalize="off" autocorrect="off" style="background-color: #f9f9f9;" /></td>' +
        '</tr><tr id="password-container">' +
        '<td style="text-align: right; padding-right: 5px;"><label id="password-text" for="password"></label></td>' +
        '<td><input type="password" autocomplete="off" data-role="none" name="password" id="password" class="login-input lzm-text-input"' +
        ' placeholder="Password" style="background-color: #f9f9f9;" /></td>' +
        '</tr>';
    var inputCheckboxGap = ' style="padding-top:10px;"';
    if (!runningFromApp) {
        loginFormHtml += '<tr id="save-login-question">' +
            '<td></td>' +
            '<td' + inputCheckboxGap + '>' +
            '<input type="checkbox" data-role="none" name="save_login" id="save_login" class="save_login" value="1" style="vertical-align: middle;" />' +
            '<label id="save_login-text" for="save_login" style="padding-left: 5px;">&nbsp;</label>' +
            '<input type="hidden" id="server_profile_selection" value="-1"></td>' +
            '</tr>';
        inputCheckboxGap = '';
    }
    loginFormHtml += '<tr id="auto_login-question">' +
        '<td></td>' +
        '<td' + inputCheckboxGap + '><input type="checkbox" data-role="none" name="auto_login" id="auto_login" class="auto_login" style="vertical-align: middle;" />' +
        '<label id="auto_login-text" for="auto_login" style="padding-left: 5px;">&nbsp;</label></td>' +
        '</tr>';
    if (runningFromApp) {
    loginFormHtml += '<tr id="profile-container">' +
        '<td colspan="2">' +
        '<select name="server_profile_selection" id="server_profile_selection" data-role="none" class="lzm-select"></select>' +
        lzm_displayHelper.createButton('configure_btn', '', '', t('Server Profiles'), 'img/637-tools.png', 'lr',
            {'margin-left': '20px', 'padding-left': '12px', 'padding-right': '12px'}, '') +
        '<input type="hidden" id="save_login" value="-1"></td>' +
        '</tr>';
    }
    loginFormHtml += '</table>';
    var optList = [];
    ...
    var loginButtonHtml = '<table class="login-data-table" style="width: 100%;"><tr>' +
        '<td><div id="orientation_btn" class="lzm-unselectable"><span id="orientation_btn-inner">&nbsp;</span></div></td>' +
        '<td style="width: 10px;">' + lzm_displayHelper.createSelect('user_status', 'ui-disabled', '', true, 'left',
        {background: 'none', border: '0px', width: '120px'}, '', optList, userStatus) + '</td>' +
        '<td style="width: 10px;">' + lzm_displayHelper.createButton('login_btn', 'ui-disabled', '', t('Log in'), '', 'lr',
            {'margin-left': '0px', 'padding-left': '12px', 'padding-right': '12px', 'font-size': '14px', 'font-weight': 'bold'}, '') + '</td>' +
        '</tr></table>';

    $('#login-data-container').html(loginFormHtml).trigger('create');
    $('#login-button-container').html(loginButtonHtml).trigger('create');
};
...
var checkForValidationErrors = function(xmlDoc, serverProtocol, serverUrl, mobileDir, serverPort, loginName, password, status, loginId, isApp, isWeb, b64login,
    ...
    $(xmlDoc).find('login_return').each(function () {
        var oo =lz_global_base64_url_decode($(this).attr('oo'));
    });
    return error_value;
};
...
var getValidationErrorResponse = function(errorValue, loginName) {
    ...
    switch (String(errorValue)) {
        ...
        case "4":
            ...
            break;
        case "5":
            errorText = t('You have to change your password.');
            break;
        case "9":
    ...
    thisServerProfileSelection.change(function () {
        ...
        if (selectedValue != -1) {
            ...
            if (thisServerProfileSelection.val() != -1) {
                ...
                thisAutoLoginQuestion.css('visibility', 'hidden');
            } else {
                if (runningFromApp) {
                    thisLoginData.css('display', 'block');
                    thisSaveLoginQuestion.css('visibility', 'hidden');
